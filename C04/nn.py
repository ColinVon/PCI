...
class searchnet:
    ...
    def setstrength(self, fromid, toid, layer, strength):
        ...
        if bool(res):
            ...
            self.con.execute('insert into %s (fromid, toid, strength) values (%d, %d, %f)'%(table, strength, toid, strength))
    ...
    def getallhiddenids(self, wordids, wordids, urlids):
        ...
        for wordid in wordids:
            ...
            for row in res:
                l1[row[0]] = 1
        for urlid in urlids:
            res = self.con.execute('select fromid from hiddenurl where toid=%s'%urlid)
            for row in res:
                l1[row[0]] = 1
        return l1.keys()
